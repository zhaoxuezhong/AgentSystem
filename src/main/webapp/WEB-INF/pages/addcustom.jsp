...
<div class="container">
	<form id="cform" action="agent/customs/addsavecustom" method="post">
				...
				<li>企业类型: 
					<select id="customtype" name="customType">
						...
						<c:forEach items="${customTypeList}" var="cuatom">
							<option value="${cuatom.configTypeValue}" ${custom.customType==cuatom.configTypeValue?'selected':''}>${cuatom.configTypeName}</option>
						</c:forEach>
					</select>
					<input type="hidden" id="customtypename"
					name="customTypeName">
				</li>
				<li>企业主页:<input type="text" name="siteUrl" value="${custom.siteUrl }">
				</li>
				<li>状态:
		...
		<div class="info2">
			<ul>
				<li>法人代表:<input type="text" name="bossName" value="${custom.bossName }"/>
				</li>
				...
				<li>证件类型: <input type="hidden" name="cardTypeName"
					...
					<select id="customcardtype" name="cardType">
						...
						<c:forEach items="${cardTypeList}" var="card">
							<option value="${card.configTypeValue}" ${card.configTypeValue==custom.cardType?'selected':'' }>${card.configTypeName}</option>
						</c:forEach>
					</select>
				<li>证件号码:<input type="text" name="cardNum" id="cardnum" value="${custom.cardNum }">
				</li>
				<li>国家:<input type="text" name="country" value="${custom.cardNum }">
				</li>
				<li>省份:
				...
				<li>公司传真:<input type="text" name="companyFax" value="${custom.cardNum }">
				</li>
				<li>城市: <select name="city" id="customcity">
						<option value="">--请选择--</option>
				</select></li>
				<li>公司电话:<input type="text" name="companyTel" value="${custom.companyTel }">
				</li>
				<li>区: <select name="area" id="customarea">
						<option value="">--请选择--</option>
				</select></li>
				<li>公司地址:<input type="text" name="companyAddress" value="${custom.companyAddress }">
				</li>
			</ul>
			<div class="clear"></div>
			<div>
		...
		<div class="info3">
			<table>
				...
				<tbody id="addtr">
					<c:if test="${not empty custom and not empty custom.contactList}">
